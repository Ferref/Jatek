...
-- 31. Egy játékos nevének és szerverének módosítása, akinek a nevében szerepel az "o" karakter
UPDATE Jatekos SET nev = 'ÚjNév', szerverId = 3 WHERE id IN (SELECT jatekosId FROM Jatekos WHERE nev LIKE '%o%');
...
UPDATE Jatekos SET online = FALSE WHERE id IN (SELECT jatekosId FROM Jatekos WHERE nev LIKE '%J%');

-- 41. Egy játékos szerverének módosítása, ahol legalább 4 felszerelés van
UPDATE Jatekos SET szerverId = 3 WHERE id IN (SELECT jatekosId FROM JatekosFelszereles GROUP BY jatekosId HAVING COUNT(*) >= 4);

-- 42. Egy kaszt sebzésModositojának növelése, ahol legalább 2 képesség van
UPDATE Kaszt SET sebzesModosito = sebzesModosito + 30 WHERE id IN (SELECT kasztId FROM Kepesseg GROUP BY kasztId HAVING COUNT(*) >= 2);

-- 43. Egy felszerelés törlése, ahol a minimumSzintje legalább 4
DELETE FROM Felszereles WHERE minimumSzint >= 4;

-- 44. Egy szörny nevének módosítása, ahol a sebzése legalább 200
UPDATE Szorny SET nev = 'ÚjNév' WHERE sebzes >= 200;

-- 45. Egy bolt nevének frissítése, ahol legalább 50 aranyat dobhatnak
UPDATE Bolt SET nev = 'ÚjBoltNév' WHERE id IN (SELECT boltId FROM Szorny WHERE aranyatDobhat >= 50);

-- 46. Egy felhasználó törlése, ahol legalább 100 aranya van
DELETE FROM Felhasznalo WHERE id IN (SELECT felhasznaloId FROM Jatekos WHERE arany >= 100);

-- 47. Egy csoport tagjainak számának csökkentése, ahol legalább 3 játékos van
UPDATE Csoport SET tagokSzama = tagokSzama - 1 WHERE id IN (SELECT csoportId FROM Jatekos GROUP BY csoportId HAVING COUNT(*) >= 3);

-- 48. Egy helyszín törlése, ahol a minimumSzintje legalább 5
DELETE FROM Helyszin WHERE minimumSzint >= 5;

-- 49. Egy szerver nevének frissítése, ahol legalább 2 csoport van
UPDATE Szerver SET nev = 'ÚjNév' WHERE id IN (SELECT szerverId FROM Csoport GROUP BY szerverId HAVING COUNT(*) >= 2);

-- 50. Visszaadja azoknak a játékosoknak a nevét és szintjét, akiknek a szintje kisebb, mint 5, és akik a "Valuri" szerveren vannak
SELECT J.nev AS jatekos_neve, J.szint
FROM Jatekos J
WHERE J.szint < 5 
AND J.szerverId = (
    SELECT S.id
